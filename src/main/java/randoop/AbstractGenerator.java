...
public abstract class AbstractGenerator {
    ...
    // Number of generation steps (one step consistents of an attempt to
    // generate and execute a new, distinct sequence)
    public int num_steps = 0;

    // Number of sequences generated.
    public int num_sequences_generated = 0;

    // Number of sequences generated that reveal a failure.
    ...
    /**
     * The list of final sequences that are printed out as JUnit tests (i.e.
     * Randoop's output).
     */
     ...
     *            component manager to use to store sequences during
     ...
     * 
     * @param testfilters
     *            List of filters to determine which sequences to output. Can be
     *            null or empty.
     */
    public AbstractGenerator(List<StatementKind> statements, long timeMillis,
            int maxSequences, ComponentManager componentManager,
            IStopper stopper, RandoopListenerManager listenerManager,
            List<ITestFilter> testfilters) {
    ...
    public void explore() {
        ...
        if (!GenInputsAbstract.noprogressdisplay) {
            progressDisplay = new ProgressDisplay(this, listenerMgr,
                    ProgressDisplay.Mode.MULTILINE, 200);
        ...
        if (!GenInputsAbstract.noprogressdisplay) {
//            System.out.println();
//            System.out.println("Normal method executions:"
//                    + ReflectionExecutor.normalExecs());
//            System.out.println("Exceptional method executions:"
//                    + ReflectionExecutor.excepExecs());
//            System.out.println();
//            System.out
//                    .println("Average method execution time (normal termination):     "
//                            + String.format("%.3g",
//                                    ReflectionExecutor.normalExecAvgMillis()));
//            System.out
//                    .println("Average method execution time (exceptional termination):"
//                            + String.format("%.3g",
//                                    ReflectionExecutor.excepExecAvgMillis()));
        }
    ...
    public Set<Sequence> subsumed_sequences() {
        throw new Error("subsumed_sequences not supported for "
                + this.getClass());
    }
}
