...
class VariableRenamer {
    ...
    public String getRenamedVar(int index) {
        ...
        if (name == null) {
            assert sequence.getStatementKind(index).getOutputType()
                    .equals(void.class) : "The index: " + index
                    + "-th output should be void.";
    ...
    private Map<Integer, String> renameVarsInSequence() {
        ...
        for (int i = 0; i < this.sequence.size(); i++) {
            Class<?> outputType = this.sequence.getStatementKind(i)
                    .getOutputType();
            if (outputType.equals(void.class)) {
                continue;
            }
            String rename = isGetter(sequence.getStatementKind(i)) ? getVariableNameForGetter(((RMethod) sequence
                    .getStatementKind(i)).getMethod())
                    : getVariableName(outputType);
            if (!name_counting_map.containsKey(rename)) {
                index_var_map.put(new Integer(i), rename);
                // update and increase the counting in name map
                name_counting_map.put(rename, 2);
            }
            else {
    ...
    private static boolean isGetter(StatementKind stmt) {
        if (stmt instanceof RMethod) {
            String name = ((RMethod) stmt).getMethod().getName();
    ...
    private static String getVariableNameForGetter(Method method) {
        String desc = method.getName().substring(3);
        if (desc.length() <= 1)
            return desc.toLowerCase();
        return desc.substring(0, 1).toLowerCase() + desc.substring(1);
    }
}
