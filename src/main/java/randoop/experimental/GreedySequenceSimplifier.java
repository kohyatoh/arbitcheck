...
public class GreedySequenceSimplifier extends AbstractSimplifier {
    ...
    public ExecutableSequence simplfy_sequence() {
        ...
        for (int i = orig_size - 1; i > -1; i--) {
            ...
            int is = super.compute_index_in_simplified_sequence(
                    simplified_sequence, removed_indice, i);
            assert is >= 0 && is < simplified_sequence.size() : "is is not legal: "
                    + is
                    + ", the simplified sequence length: "
            ...
            for (int input_index = 0; input_index < inputVars.size(); input_index++) {
                ...
                int iInSimplified = super.compute_index_in_simplified_sequence(
                        simplified_sequence, removed_indice, i);
                List<Integer> compatible_indices = this
                        .findCompatibleTypeIndicesInReverseOrder(
                                simplified_sequence, iInSimplified,
                                var.getType());
                if (compatible_indices.isEmpty()) {
                    continue;
                }
                else {
                    // 1. replace var with variables produced by statement in
                    // compatible_indices,
                    // and then get the smallest one
                    int indexInSimplified = super
                            .compute_index_in_simplified_sequence(
                                    simplified_sequence, removed_indice, i);
                    Sequence s = this
                            .replaceVarWithSmallestCompatibleIndex(
                                    compatible_indices, removed_indice,
                                    simplified_sequence, indexInSimplified,
                                    input_index);
                    // 2. remove all unused vars, then keep going
                    if (s != null) {
                        simplified_sequence = s;
                        // then remove all unused vars
                        RedundantStatementRemover unused_simplifier = new RedundantStatementRemover(
                                simplified_sequence, this.visitor);
                        Sequence tmp_sequence = unused_simplifier
                                .simplfy_sequence().sequence;
                        List<Integer> remove_indices_in_orig = new LinkedList<Integer>();
                        for (int removed_index_in_simplified : unused_simplifier
                                .getRemovedIndices()) {
                            remove_indices_in_orig.add(super
                                    .compute_index_in_original_sequence(
                                            simplified_sequence,
                                            removed_indice,
        ...
        ExecutableSequence ret_eseq = new ExecutableSequence(
                simplified_sequence);
     ...
     * statements in <code>replacable_var_index</code> to as the
     * <code>input_index</code>-th input of the <code>stmt_num</code>-th
     * statement.
     * */
    protected Sequence replaceVarWithSmallestCompatibleIndex(
            List<Integer> replacable_var_index, List<Integer> removed_indices,
            Sequence sequence, int stmt_num, int input_index) {
        ...
        for (int i : replacable_var_index) {
            ...
            msequence.getInputs(stmt_num).add(input_index,
                    msequence.getVariable(replace_var.index));
            ...
            if (!eSeq.hasFailure()) {
                ...
                int failure_index_in_orig = super
                        .compute_index_in_original_sequence(sequence,
                                removed_indices, failure_index);
                if (failure_index_in_orig == super.getFailureIndex()
                        && super.compareFailureChecks(super.getFailureChecks(),
                                eSeq.getFailures(failure_index))) {
    ...
    /**
     * Finding a list output-type-compatible statements in the sequence, between
     * range [0, maxIndex)
     */
    protected List<Integer> findCompatibleTypeIndicesInReverseOrder(
            Sequence sequence, int maxIndex, Class<?> type) {
        assert maxIndex >= 0 && maxIndex < sequence.size() : "The max index: "
                + maxIndex + " is not correct for sequence size: "
                + sequence.size();
