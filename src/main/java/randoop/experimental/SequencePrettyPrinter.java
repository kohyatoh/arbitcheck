...
public class SequencePrettyPrinter {
    ...
    public SequencePrettyPrinter(List<ExecutableSequence> seqs,
            String packageName, String clzName, String methodName) {
    ...
    public String prettyPrintSequences() {
        ...
        sb.append(Globals.lineSep);
        sb.append("import static arbitcheck.TestSupport.checkProperty;");
        sb.append(Globals.lineSep);
        sb.append(Globals.lineSep);
        ...
        for (ExecutableSequence eseq : this.outputSequences) {
            ...
            sb.append(Globals.lineSep);
            sb.append(indent("public void " + methodName + (count++)
                    + "() throws Throwable {", 2));
            sb.append(Globals.lineSep);
            // sb.append(Globals.lineSep);
    ...
    private String[] extractImportClasses() {
        ...
        for (ExecutableSequence eseq : this.outputSequences) {
            ...
            for (int i = 0; i < length; i++) {
                ...
                if (statement instanceof RMethod) {
                    ...
                    if (rmethod.isStatic()) {
                        ...
                        if (needImport(declaring_class)) {
                            import_clz_set
                                    .add(convertToCompilableNames(declaring_class));
                        }
                    }
                ...
                if (statement instanceof RMonitoring) {
                    import_clz_set
                            .add(convertToCompilableNames(Monitoring.class));
                    import_clz_set
                            .add(convertToCompilableNames(DummyMonitoring.class));
                }
            }
        }
