...
public class SequenceSimplifyUtils {
     ...
     * used in assertions.
     * */
    public static List<Integer> getAllUnusedVariableIndexAsList(
    ...
    /**
     * Checks if the index-th statement can be removed safely without breaking
     * the whole program.
     * */
    public static boolean isStatementRemovable(Sequence original_sequence,
            int index) {
        int size = original_sequence.size();
        assert index >= 0 && index < size : "The index: " + index
                + " is not valid. given the sequence length: " + size;
    ...
    public static int computeIndexInOriginalSequence(
            Sequence sequenceToSimplify, Sequence simplifiedSequence,
        ...
        assert simplifiedSequence.size() + removed_indices.size() == sequenceToSimplify
    ...
    /**
     * retains the statements. XXX NOTE,this may return null, if the given index
     * list is not valid to construct a sequence
     * */
    public static Sequence retainStatements(Sequence original_sequence,
            List<Integer> retained) {
        for (int index : retained) {
            assert index > -1 && index < original_sequence.size() : "The index is not valid: "
                    + index
        ...
        for (int i = 0; i < retained.size(); i++) {
            String name = original_sequence.getVariable(retained.get(i) /*
                                                                         * index
                                                                         * in
                                                                         * the
                                                                         * original
        ...
        for (int i = 0; i < retained.size(); i++) {
            ...
            for (MVariable v : mstatement.inputs) {
                ...
                // get the variable
                int indexOfRetained = retained
                        .indexOf(indexInOriginalStatement);
                MVariable mv = newvars.get(indexOfRetained);
                newinputs.add(mv);
            }
            statements.add(new MStatement(mstatement.statementKind, newinputs,
                    newvars.get(i)));
        }
    ...
    public static Sequence removeStatement(Sequence original_sequence, int index) {
        ...
        for (int i = 0; i < new_size; i++) {
            String name = i < index ? original_sequence.getVariable(i)
                    .getName() : original_sequence.getVariable(i + 1).getName();
