...
public class HelperSequenceCreator {
        ...
        if (cls.getComponentType().isPrimitive()) {
            ...
            if (candidates.isEmpty()) {
                if (GenInputsAbstract.forbid_null) {
                    ...
                    // No sequences that produce appropriate component values
                    // found, and null allowed.
                    // TODO: We should also randomly return the empty
                    // array--it's a perfectly good case
                    // even if null is allowed.
                    // Return the array [ null ].
                    ArrayDeclaration decl = new ArrayDeclaration(
                            cls.getComponentType(), 1);
                    s = new Sequence();
                    s = s.extend(PrimitiveOrStringOrNullDecl.nullOrZeroDecl(cls
                            .getComponentType()));
                ...
                // statement will have such a var,
                // which I know is currently true because of SequenceCollection
                // implementation.
                ins.add(s.randomVariableForTypeLastStatement(
    ...
    private static Sequence randPrimitiveArray(Class<?> componentType) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            s = s.extend(new PrimitiveOrStringOrNullDecl(componentType, elt),
                    emptylist);
        }
