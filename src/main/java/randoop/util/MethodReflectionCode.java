...
public final class MethodReflectionCode extends ReflectionCode {
    ...
    private void checkRep() {
        if (!GenInputsAbstract.debug_checks)
            return;
        String error = Reflection.checkArgumentTypes(inputs,
        ...
        if (Modifier.isStatic(this.method.getModifiers())) {
            ...
            if (!Reflection.canBePassedAsArgument(receiver,
                    method.getDeclaringClass()))
                throw new IllegalArgumentException("method " + method
                        + " cannot be invoked on " + receiver);
        }
        // TODO check that the lookup starting at receiver.getClass<?> will
        // result
        // in method
    }

    @Override
    public void runReflectionCodeRaw() throws IllegalAccessException,
