...
/**
 * Immutable. Represents a one-dimensional array creation statement, e.g.
 * "int[] x = new int[2] { 3, 7 };"
 */
public final class ArrayDeclaration implements StatementKind, Serializable {
    ...
    public ArrayDeclaration(Class<?> elementType, int length) {
        ...
        if (length < 0)
            throw new IllegalArgumentException(
                    "arity cannot be less than zero: " + length);
    ...
    public String toString() {
        return "array_of_" + this.elementType.getSimpleName() + "_of_size_"
                + this.length;
    }
    ...
    public void appendCode(Variable newVar, List<Variable> inputVars,
        ...
        for (int i = 0; i < inputVars.size(); i++) {
            ...
            if (!GenInputsAbstract.long_format
                    && ExecutableSequence
                            .canUseShortFormat(statementCreatingVar)) {
                b.append(PrimitiveTypes
     ...
     * TYPE[NUMELEMS]
     * 
     * Where TYPE is the type of the array, and NUMELEMS is the number of
     * elements.
     * 
     * Example:
     * 
