...
public abstract class GenInputsAbstract extends CommandHandler {
    ...
    public GenInputsAbstract(String command, String pitch,
            String commandGrammar, String where, String summary,
            List<String> notes, String input, String output, String example,
            Options options) {
     ...
     * The list of methods given by this argument will augment any methods
     * derived via the --classlist option.
     * 
     * Also see the manual section on <a href=
     * "http://randoop.googlecode.com/hg/doc/index.html#specifying-methods"
     * >specifying methods and constructors under test</a>.
     */
     ...
     * classes/methods are considered visible. If public_only is false then any
     * class/method that is not private is considered visible.
     * 
     * FIXME: This option outputs tests that do not compile. Until a fix is
     * done, keep the option @Unpublicized. The option should probably be an
     * enum with PUBLIC, PACKAGE, PUBLIC elements.
     */
    @Unpublicized
     ...
     * (--inputlimit=int) is reached. The number of tests output may be smaller
     * than then number of inputs created, because redundant and illegal inputs
     * may be discarded. Also see --outputlimit.
     */
    @Option("Maximum number of tests generated")
    public static int inputlimit = 100000000;

    /**
     * Determines the maximum number of tests to output, no matter how many are
     * generated. Contrast to --inputlimit.
     */
     ...
     * type is available.
     * 
     * To ask Randoop to calls methods with null with greater frequency, see
     * option --null-ratio.
     */
     ...
     * 
     * If a null ratio is given, it should be between 0 and 1. A ratio of X
     * means that null will be used instead of a non-null value as a parameter
     * to method calls, with X frequency (1 means always use null, 0 means never
     * use null). For example, a ratio of 0.5 directs Randoop to use null inputs
    ...
    /**
     * Clear the component set each time it contains the given number of inputs. <br />
    ...
    public static boolean simplify_failed_tests = true;
    ...
    /** Base name of the JUnit test case method */
    @Option("Base name of the JUnit test case(s)")
    public static String junit_methodname = "test";
    ...
    @Option("Exception classes represents assumption violations")
    public static String assumption_classes = "org.junit.internal.AssumptionViolatedException";

    public static List<Class<?>> assumption_class_list = new ArrayList<Class<?>>();

    /**
     * Output sequences even if they do not complete execution.
     * 
     * Randoop's default behavior is to output only tests consisting of method
     * call sequences that execute to the end, rather than throwing an exception
     * or failing a contract check in the middle of execution.
     */
    ...
    /**
     * Whether to use the long format for outputting JUnit tests. The long
     * format emits exactly one line per statement, including primitive
     * declarations, and uses boxed primitives. This option is used in the
     * branch-directed generation project.
     */
    @Unpublicized
    ...
    @Option("Check property * times")
    public static int check_count = 1000;
    ...
    public void checkOptionsValid() {
        ...
        if (!(output_tests.equals(all) || output_tests.equals(pass) || output_tests
                .equals(fail))) {
        ...
        if (maxsize <= 0) {
            throw new RuntimeException(
                    "Maximum sequence size must be greater than zero but was "
                            + maxsize);
        }
    }
    ...
    List<Class<?>> findClassesFromArgs(Options printUsageTo) {
        ...
        try {
            ...
            String msg = Util.toNColsStr(
                    "ERROR while reading list of classes to test: "
                            + e.getMessage(), 70);
