...
public class ForwardGenerator extends AbstractGenerator {
    ...
    public ForwardGenerator(List<StatementKind> statements, long timeMillis,
            int maxSequences, ComponentManager componentManager,
            IStopper stopper, RandoopListenerManager listenerManager,
            List<ITestFilter> fs) {

        super(statements, timeMillis, maxSequences, componentManager, stopper,
                listenerManager, fs);
    ...
    public ExecutableSequence step() {
        ...
        if (eSeq.sequence.hasActiveFlags()) {
            // Optimization -- don't use objects checked to extend sequence
            if (!SequenceUtil.isChecked(eSeq.sequence)) {
    ...
    public void processSequence(ExecutableSequence seq) {
        ...
        for (int i = 0; i < seq.sequence.size(); i++) {
            ...
            if (runtimeValue == null) {
                if (Log.isLoggingOn()) {
                    Log.logLine("Making index " + i
                            + " inactive (value is null)");
                }
            ...
            if (this.allSequences.contains(newSequence)) {
                ...
                if (this.allsequencesAsCode.contains(code)) {
                    ...
                    b.append("new component:" + Globals.lineSep + ""
                            ...
                            + Globals.lineSep);
                    b.append("existing component:" + Globals.lineSep + ""
    ...
    private InputsAndSuccessFlag selectInputs(StatementKind statement) {
        ...
        for (int i = 0; i < inputTypes.size(); i++) {
            ...
            if (Monitoring.class.equals(t)) {
                variables.add(totStatements);
                sequences.add(Sequence.create(new RMonitoring()));
                totStatements += 1;
                continue;
            }
            ...
            if (GenInputsAbstract.alias_ratio != 0
                    && Randomness
                            .weighedCoinFlip(GenInputsAbstract.alias_ratio)) {

                // candidateVars will store the indices that can serve as input
                // to the i-th input in st.
                ...
                // the indices in S of type T.
                for (Class<?> match : types.getMatches(t)) {
                    // Sanity check: the domain of typesToVars contains all the
                    // types in variable types.
                    assert typesToVars.keySet().contains(match);
                    candidateVars
                            .add(new ArrayListSimpleList<Integer>(
                                    new ArrayList<Integer>(typesToVars
                                            .getValues(match))));
                }

                // If any type-compatible variables found, pick one at random as
                // the i-th input to st.
                SimpleList<Integer> candidateVars2 = new ListOfLists<Integer>(
                        candidateVars);
                if (candidateVars2.size() > 0) {
                    int randVarIdx = Randomness.nextRandomInt(candidateVars2
                            .size());
            ...
            // We use one of three ways to gather candidate sequences, but the
            // third case below
            // is by far the most common.

            if (GenInputsAbstract.always_use_ints_as_objects
                    && t.equals(Object.class)) {

                // 1. OBSCURE, applicable only for branch-directed generation
                // project. Get all
                // sequences that create one or more integer. Applicable only
                // when inputTypes[i]
                // is "Object" and always_use_ints_as_objects option is
                // specified.
                if (Log.isLoggingOn())
                ...
                // 2. If T=inputTypes[i] is an array type, ask the component
                // manager for all sequences
                // of type T (list l1), but also try to directly build some
                // sequences that create arrays (list l2).
                SimpleList<Sequence> l1 = componentManager.getSequencesForType(
                        statement, i);
                if (Log.isLoggingOn())
                    Log.logLine("Array creation heuristic: will create helper array of type "
                            + t);
                ...
                // 3. COMMON CASE: ask the component manager for all sequences
                // that yield the required type.
                if (Log.isLoggingOn())
                    Log.logLine("Will query component set for objects of type"
                            + t);
            ...
            // inputs with some given frequency.
            ...
            // chosenSeq.randomVariableForTypeLastStatement(t, m);
            Variable randomVariable = chosenSeq.randomVariableForType(t, m);
            ...
            if (randomVariable == null) {
                throw new BugInRandoopException("type: " + t + ", sequence: "
                        + chosenSeq);
            }
