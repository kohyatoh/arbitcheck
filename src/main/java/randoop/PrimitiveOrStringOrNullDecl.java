 ...
 * doesn't actually transform any state, but it works out nicely to represent
 * primitives as statements.
 * 
...
public final class PrimitiveOrStringOrNullDecl implements StatementKind,
    ...
    public PrimitiveOrStringOrNullDecl(Class<?> t, Object o) {
        ...
        if (t.isPrimitive()) {
            ...
            if (o != null) {
                throw new IllegalArgumentException(
                        "value must be null for non-primitive, non-string type "
                                + t + " but was " + o);
            }
        }
    ...
    public void appendCode(Variable newVar, List<Variable> inputVars,
        ...
        if (!varUsed) {
            // append no line if the variable is not used
            return;
        }
    ...
    public static PrimitiveOrStringOrNullDecl nullOrZeroDecl(Class<?> c) {
        ...
        if (Integer.TYPE.equals(c))
            return new PrimitiveOrStringOrNullDecl(Integer.TYPE,
                    (Integer.valueOf(0)).intValue());
        if (Long.TYPE.equals(c))
            return new PrimitiveOrStringOrNullDecl(Long.TYPE,
                    (Long.valueOf(0)).longValue());
        if (Float.TYPE.equals(c))
            return new PrimitiveOrStringOrNullDecl(Float.TYPE,
                    (Float.valueOf(0)).floatValue());
        if (Double.TYPE.equals(c))
            return new PrimitiveOrStringOrNullDecl(Double.TYPE,
    ...
    public static Sequence sequenceForPrimitive(Object o) {
        ...
        if (cls.equals(String.class)
                && !PrimitiveTypes.stringLengthOK((String) o)) {
            throw new IllegalArgumentException("o is a string of length > "
                    + GenInputsAbstract.string_maxlen);
        }

        return Sequence.create(new PrimitiveOrStringOrNullDecl(PrimitiveTypes
                .primitiveType(cls), o));
    }
     ...
     * Examples:
     * 
     * String:null represents: String x = null java.lang.String:"" represents:
     * String x = ""; String:"" represents: String x = ""; String:" "
     * represents: String x = " "; String:"\"" represents: String x = "\"";
     * String:"\n" represents: String x = "\n"; String:"\u0000" represents:
     * String x = "\u0000"; java.lang.Object:null represents: Object x = null;
    ...
    public static PrimitiveOrStringOrNullDecl parse(String s)
        ...
        if (typeString.matches(".*\\s+.*")) {
            String msg = "Error when parsing type/value pair "
                    + s
                    + ". A primitive value declaration description must be of the form "
                    + "<type>:<value>" + " but the <type> description \"" + s
        ...
        if (type == null) {
            String msg = "Error when parsing type/value pair "
                    + s
                    + ". A primitive value declaration description must be of the form "
                    + "<type>:<value>" + " but the <type> given (\""
                    + typeString + "\") was unrecognized.";
            throw new StatementKindParseException(msg);
        }
        Object value = null;
        ...
        if (type.equals(char.class)) {
            try {
                ...
                String msg = "Error when parsing type/value pair "
                        + s
                        + ". A primitive value declaration description must be of the form "
                        + "<type>:<value>" + " but the <value> given (\""
                        + valString + "\") was not parseable.";
                throw new StatementKindParseException(msg);
            }
        }
            ...
            if (valString.equals("null")) {
                ...
                if (valString.charAt(0) != '"'
                        || valString.charAt(valString.length() - 1) != '"') {
                    String msg = "Error when parsing type/value pair "
                            + s
                ...
                if (!PrimitiveTypes.stringLengthOK((String) value)) {
                    throw new StatementKindParseException(
            ...
            if (valString.equals("null")) {
                ...
                String msg = "Error when parsing type/value pair "
                        ...
                        + "<type>:null but the string given (\"" + valString
