...
public class ExecutableSequence implements Serializable {
     ...
     */
    public ExecutableSequence(Sequence sequence, Execution exec,
            List<List<Check>> checks) {
        this.sequence = sequence;
    ...
    /**
     * Adds the given check to the i-th element of the sequence. Only one check
     * of class StatementThrowsException is allowed for each index, and
     * attempting to add a second check of this type will result in an
    ...
    public int seq_id() {
        ...
        for (int i = 0; i < sequence.size(); i++) {
            ...
            if (!GenInputsAbstract.long_format
                    && ExecutableSequence.canUseShortFormat(sequence
                            .getStatementKind(i))) {
                continue;
            }
    ...
    public String toCodeString() {
        // mark variables used in later statements
        boolean[] varUsed = new boolean[sequence.size()];
        for (int i = 0; i < sequence.size(); i++) {
            for (int index : sequence.getInputsAsAbsoluteIndices(i)) {
                varUsed[index] = true;
            }
        }
        ...
        for (int i = 0; i < sequence.size(); i++) {
            ...
            StringBuilder oneStatement = new StringBuilder();
            sequence.printStatement(oneStatement, i, varUsed[i]);
     ...
     * 
     * @param visitor
     *            can be null, in which case no visitor will be invoked during
     *            execution.
     */
    public void execute(ExecutionVisitor visitor, boolean stop_on_exception) {
        ...
        for (int i = 0; i < this.sequence.size(); i++) {
            ...
            // If a null value is implicitly passed to the statement (e.g., to
            // the receiver)
            // stop execution.
            if (!getRuntimeInputs(sequence, executionResults.theList, i,
                    inputs, inputVariables))
                break;

            executeStatement(sequence, executionResults.theList, i,
                    inputVariables);
    ...
    public static boolean getRuntimeInputs(Sequence s,
            List<ExecutionOutcome> outcome, int i, List<Variable> inputs,
            Object[] runtimeObjects) {
        ...
        for (int ri = 0; ri < runtimeObjects.length; ri++) {
            if (runtimeObjects[ri] == null) {
                ...
                StatementKind creatingStatement = s
    ...
    protected static Object[] getRuntimeValuesForVars(List<Variable> vars,
            List<ExecutionOutcome> execution) {
        ...
        for (int j = 0; j < runtimeObjects.length; j++) {
            ...
            assert execution.get(creatingStatementIdx) instanceof NormalExecution : execution
                    .get(creatingStatementIdx).getClass();
            NormalExecution ne = (NormalExecution) execution
    ...
    public static void executeStatement(Sequence s,
            List<ExecutionOutcome> outcome, int index, Object[] inputVariables) {
        ...
        synchronized (ProgressDisplay.print_synchro) {
            ...
            assert ((statement instanceof RMethod && !((RMethod) statement)
                    .isStatic()) ? inputVariables[0] != null : true);
    ...
    /**
     * @return the first index at which an check of the given type occurs, or -1
     *         if there is no check of the given type in this sequence.
     */
    ...
    /**
     * True iff this sequences has at least one check of the given type at index
     * i.
     */
    ...
    public static <D extends Check> List<Sequence> getSequences(
    ...
    @Override
    public int hashCode() {
        return sequence.hashCode() * 3 + checks.hashCode() * 5;
    ...
    /**
     * Compares the results of the checks of two sequences (the code in each
     * sequence should be the same). Returns the number of different checks. If
     * remove_diffs is true any differing checks are removed from both
     * sequences. Checks found in one sequence but not the other are also
     * removed. Prints any differences to stdout if print_diffs is true.
     * 
     * Different checks can be generated for two runs of the same test sequence
     * because of changes in the environment (eg, static variables) between
     * runs. Some checks are only created when the value involved in the check
     * meets certain criteria and since the values can change, the checks may
     * not always be created. Its also possible to throw exceptions in one run
     * and not another.
     * 
     * The get_id() method from the Check interface) is used to identify check
     * uniquely. The id contains both the observer type (primitive, non-null,
     * observer method call, etc) and the related variables. When comparing two
     * lists of Checks, the code presumes that the checks will occur in the same
     * order on each execution. Thus if the first remaining check in the first
     * execution matches the second check from the second execution, the first
     * check from the second execution is presumed to not match. The algorithm
     * is far from completely optimized, but it should work pretty well if the
     * two lists are pretty much the same (which they usually are).
     */
    public int compare_checks(ExecutableSequence es, boolean remove_diffs,
        ...
        for (int ii = 0; ii < checks.size(); ii++) {
            ...
            for (int o1i = 0; o1i < obs1.size(); o1i++) {
                ...
                if (o2i >= obs2.size()) {
                    diff_obs1.add(0, o1i);
                    print_diffs(print_diffs, "extra obs1", ob1, null, o1i, -1,
                            es);
                }
                boolean match = false;
                int o2i_start = o2i;
                while (o2i < obs2.size()) {
                    ...
                    if (ob1.get_id().equals(ob2.get_id())) {
                        ...
                        if (!ob1.get_value().equals(ob2.get_value())) {
                            ...
                            diff_obs2.add(0, o2i);
                            cnt++;
                            print_diffs(print_diffs, "mismatch", ob1, ob2, o1i,
                                    o2i, es);
                        }
                        else { // values match
                            if (debug)
                                print_diffs(print_diffs, "match", ob1, ob2,
                                        o1i, o2i, null);
                        }
                        match = true;
                        break;
                ...
                // If we found a match any skipped checks in obs2 must be
                // non-matched
                if (match) {
                    for (int jj = o2i_start; jj < o2i; jj++) {
                        diff_obs2.add(0, jj);
                        print_diffs(print_diffs, "obs2 missing", null,
                                obs2.get(jj), o1i, o2i, es);
                    }
                    o2i++;
                }
                else { // no match for obs1
                    diff_obs1.add(0, o1i);
                    print_diffs(print_diffs, "obs1 missing", ob1, null, o1i,
                            -1, es);
                    o2i = o2i_start;
                }
            }
            for (; o2i < obs2.size(); o2i++) {
                diff_obs2.add(0, o2i);
                print_diffs(print_diffs, "extra obs2", null, obs2.get(o2i), -1,
                        o2i, es);
            }
    ...
    private void print_diffs(boolean print_diffs, String msg, Check obs1,
        ...
        if (es != null) {
            ...
            System.out.printf("Seq 2 [%08X]%n%s%n", es.seq_id(),
                    es.toCodeString());
        }
    }
